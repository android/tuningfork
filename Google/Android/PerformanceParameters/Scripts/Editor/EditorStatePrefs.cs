...
namespace Google.Android.PerformanceParameters.Editor
{
    ...
    {
        private const string KeyPrefix = "tuningfork-cache-";
        private readonly string m_Key;
        private readonly T m_DefaultValue;

        public EditorStatePrefs(string key, T defaultValue)
        {
            this.m_Key = KeyPrefix + key;
            this.m_DefaultValue = defaultValue;
        }

        public T Get()
        {
            if (!EditorPrefs.HasKey(m_Key)) Set(m_DefaultValue);
            var json = EditorPrefs.GetString(m_Key);
