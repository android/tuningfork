...
namespace Google.Android.PerformanceParameters.Editor
{
    internal class ProtocCompiler
    {
        ...
        internal static bool GenerateProtoAndDesc()
        {
        ...
        private static bool CompileProto(string protoFilePath, string[] includePaths, string outputPath)
        {
        ...
        private static bool CreateDescriptor(string protoFilePath, string outputPath)
        {
            if (Path.GetExtension(protoFilePath) != ".proto") return false;
            if (!File.Exists(binaryPath))
            {
