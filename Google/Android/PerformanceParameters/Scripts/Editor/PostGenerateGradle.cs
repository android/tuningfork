...
namespace Google.Android.PerformanceParameters.Editor
{
    ...
    /// PostGenerateGradle.
    /// </summary>
    class PostGenerateGradle : IPostGenerateGradleAndroidProject
    {
        ...
        public void OnPostGenerateGradleAndroidProject(string path)
        {
            ...
            {
                Debug.Log("Tuningfork is enabled, copying files to android project...");
                FileUtil.CopyTuningforkFilesToAndroidProject(path);
                if (!setupConfig.UseAdvancedFidelityParameters)
                {
                    Debug.Log("Tuningfork is using default fidelity parameters, generating default fidelity levels...");
                    FileUtil.SaveFidelityMessagesInApk(path, EditorUtil.FidelityMessagesWithQualityLevels());
                }
            }
        }
    }
